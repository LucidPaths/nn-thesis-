...
def load_lineage():
    """Load lineage file, returning None if it doesn't exist."""
...
def merge_from_adapter_lineage(tune_epoch=None):
    lineage = load_lineage()

    # LINEAGE MODE: Rebuild from accepted adapters only
    if lineage and lineage.get("adapters"):
        print("\n" + "=" * 70)
        print("REBUILDING FROM LINEAGE")
        print("=" * 70)
        ...
        print(f"Accepted: {[a['epoch'] for a in accepted]}")
        print("=" * 70 + "\n")
        ...
        for i, adapter_info in enumerate(accepted):
            ...
            epoch_path = Path(adapter_info["path"])

            print(f"[{i + 1}/{len(accepted)}] Merging A{epoch}")

            adapter_configs = [p for p in epoch_path.rglob("adapter_config.json") if "synth_nn" not in str(p)]
            if not adapter_configs:
                print(f"⚠ A{epoch} not found, skipping")
                continue

            adapter_dir = adapter_configs[0].parent

            if i == 0:
                parent_dir = Path(base_model).parent
                add_tokenizer(parent_dir, llm_tokenizer_out / parent_dir.name, nngpt_upload, model_name)

            merge(current_model_path, str(adapter_dir), str(output_path))
            current_model_path = str(output_path)

        print(f"\n✓ Lineage merge complete → {output_path}\n")
        return

    # FALLBACK: Single adapter merge
    print("\n[FALLBACK] No lineage, merging single adapter\n")

    epoch_root = nngpt_dir / "llm" / "epoch"

    if tune_epoch is None:
        epochs = sorted([p for p in epoch_root.glob("A*") if p.is_dir()], key=lambda x: int(x.name[1:]))
        epoch_path = None
        for ep in reversed(epochs):
            if list(ep.rglob("adapter_config.json")):
                epoch_path = ep
                break
        if not epoch_path:
            raise RuntimeError("No adapters found")
    else:
        epoch_path = epoch_root / f"A{tune_epoch}"

    adapter_configs = [p for p in epoch_path.rglob("adapter_config.json") if "synth_nn" not in str(p)]
    if not adapter_configs:
        raise RuntimeError(f"No adapter in {epoch_path}")
    ...
    add_tokenizer(nngpt_model, llm_tokenizer_out, nngpt_upload, model_name)

    merge_base = str(output_path) if output_path.exists() else base_model
    merge(merge_base, str(adapter_dir), str(output_path))
